...
const Productspecific = () => {
    ...
    document.title="Lynxi cart";
        ...
        async function fetchdata(){
          ...
          try{
            ...
            const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/products?category=${product}`,{headers:{"Content-Type":"application/json"},withCredentials:true})
  ...
  function handleaddcartclick(productid, quantity){
      ...
      axios.post(`${process.env.REACT_APP_API_URL}/api/cart/item/add`,{"productid":productid, "quantity":quantity},{headers:{"Content-Type":"application/json"},withCredentials:true}).then((response)=>{setcartresponse(response.data.message); setquantityupdated(response.data.count)}).catch((error)=>setcartresponse(error.data.message));
